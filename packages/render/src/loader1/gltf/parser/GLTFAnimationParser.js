...
import {
  ...
  AnimationSamplerInterpolation} from "../GLTFSchema.js";
...
export class GLTFAnimationParser extends GLTFParser {
   ...
   * @internal
   */
  static _parseStandardProperty(context, animationClip, animationInfo) {
    ...
    const sampleDataCollection = new Array();
      ...
      case AnimationChannelTargetPath.WEIGHTS: {
        ...
        for (let i = 0, n = input.length; i < n; i++) {
          ...
          if (curve.interpolation === InterpolationType.CubicSpine) {
            ...
            offset += outputSize;
            keyframe.value = output.subarray(offset, offset + outputSize);
            offset += outputSize;
  ...
  parse(context) {
    ...
    const animationsIndices = new Array(animationClipCount);
